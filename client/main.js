...
const referenceYearEl = document.getElementById('reference-year');
...
loginBtnLink.addEventListener('click', () => window.location.href = "/login");
...
function prepareTurn() {
  ...
  referenceYearEl.textContent = `Your reference year is ${randomYear}`;
  infoEl.textContent = `${players[currentPlayer]}'s turn! Press GO!`;
...
function startTurn() {
    ...
    const musicDuration = secondsPerDifficulty[difficulty];
    countdownEl.textContent = musicDuration;
    referenceYearEl.textContent = `Your reference year is ${assignedYears[currentPlayer]}`;
  ...
  function startAnswerTimer() {
    ...
    referenceYearEl.textContent = `Is it earlier or later than ${assignedYears[currentPlayer]}?`;
  ...
  function renderScoreboard() {
    const scoreboardData = players.map((player, idx) => ({
      name: player,
      score: scores[idx],
      index: idx
